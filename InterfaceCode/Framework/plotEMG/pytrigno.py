...
class TrignoEMG(_BaseTrignoDaq):
    ...
    def __init__(self, channel_range, samples_per_read, units='V',
                 host='localhost', cmd_port=50040, data_port=50041, timeout=10, buffered = False):
        ...
        self.buffer = np.zeros((channel_range[1] - channel_range[0] + 1,0))
        self.recordFlag = False
        self.data = np.zeros((channel_range[1] - channel_range[0] + 1,0))
        self.rate = 2000
        self.buffered = buffered
        self.scaler = 1.

        self.features = np.zeros((channel_range[1] - channel_range[0] + 1, 0))
    ...
    def read(self):
        ...
        if self.buffered:
            while self.recordFlag:
                ...
                self.buffer = np.hstack((self.buffer, self.scaler * data))
        else:
                self.data = super(TrignoEMG, self).read(self.samples_per_read)
                self.data = self.data[self.channel_range[0]:self.channel_range[1]+1, :]
    
    def flush(self):
        self.buffer = np.zeros((channel_range[1] - channel_range[0] + 1,0))
    
    def getFeatures(self):
        window_length = int(0.2*self.rate)
        window_num = 0
        while self.recordFlag:
            if self.buffer.shape[1] > window_length + self.samples_per_read*window_num:
                window = self.buffer[:,  self.samples_per_read*window_num: window_length + self.samples_per_read*window_num]
                self.features = np.hstack((self.features, np.reshape(EMGFeatures().get_mav(window.T).T,(-1,1))))
                window_num += 1
